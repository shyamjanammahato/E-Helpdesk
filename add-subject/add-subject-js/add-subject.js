$(function() {
 ...
 $('#add_subject_submit').click(function(event) {
    ...
    else
    {
      $.ajax({
      type: 'POST',
      url: 'add-subject-code/add-subject-code.php',
      data: ({ course_id: course_id , department_id: department_id , batch_id: batch_id , subject_nm: subject_nm , subject_code: subject_code , sem_id: sem_id}),
      ...
      {
        setTimeout(function(){
          $("#error_id").html("<div class='alert alert-success alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Success!</strong> Subject Added Successfully.</div>").fadeIn("fast");
        }, 1000);

        setTimeout(function(){
         document.location.href='../add-subject/index.php';
       }, 1800);
      }
      else
      {
