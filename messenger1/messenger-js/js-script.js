function chat_start(uid){
  ...
  $("#msgarea_chat").load("jquery-pages/chat-reload.php?userid="+uid).fadeIn("fast");
}
...
function send_btn()
{
  ...
  if (rusername != '')
  {
    ...
    //alert HTTP server error
    setTimeout(function(){
          $("#error_id").html("<div class='alert alert-danger alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Error!</strong> err.statusText.</div>").fadeIn("fast");
        }, 100);
    });
  }
  else
  {
   setTimeout(function(){
          $("#error_id").html("<div class='alert alert-danger alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Error!</strong> Please Select any user to chat.</div>").fadeIn("fast");
        }, 100);
  }
}
