$(function() {
 ...
 $('#add_course').click(function(event) {
    ...
    else
    {
      $.ajax({
      type: 'POST',
      url: 'add-course-code/add-course-code.php',
      data: ({ course_name: course_name , course_code: course_code }),
      ...
      {
        setTimeout(function(){
          $("#error_id").html("<div class='alert alert-success alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Success!</strong> Course Added Successfully.</div>").fadeIn("fast");
        }, 100);
